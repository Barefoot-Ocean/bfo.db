...
> 🛡️ **Security:** Your database **host, user, and password are
> secrets.** Keep them in a `config.yml` or `.Renviron` file that is
> **never committed to git** (add it to `.gitignore`). The SSL
> certificate is *not* a secret — see [SSL
> certificate](#id_-ssl-certificate-sslrootcert) below.
...
**Certificate Authority bundle** to verify the server’s identity. This
bundle is **public** (published by AWS) — it is **not** a credential and
contains no secrets.

For convenience, the AWS CA bundles ship **inside this package**, so by
default you don’t need to download or manage any `.pem` file. When you
omit `sslrootcert` (or `ssl_key` in `config.yml`), the package
automatically uses the **global** bundle (which covers every AWS
...
You only need to set `sslrootcert` (or `ssl_key`) yourself if you want
to pin a specific certificate or supply your own. The latest bundles are
always available from AWS:
